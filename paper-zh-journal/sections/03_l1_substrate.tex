...
\node[dashbox, left=7mm of heal, minimum height=12mm, minimum width=24mm, fill=gray!18] (rt) {\textbf{L2} 运行时\\DOM 捕获\\\scriptsize （后续工作）};

\draw[arrow] (ast) -- (intent);
\draw[arrow] (intent) -- (cal);
\draw[arrow] (cal.south) |- (sheet.west);
\draw[arrow, dashed] (rt.east) -- (heal.west);
