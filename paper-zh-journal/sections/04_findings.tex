...
下文所有数字都可由开放脚本 \texttt{heal\_baseline.py}、\texttt{false\_heal\_probe.py} 与 \texttt{cross\_app\_probe.py} 在 \texttt{healreact/bench/cases/} 下的 JSON 工件上复现。完整实验扫描（两个 L3 变体 + 两个 false-heal 探针）在 Apple M5 Pro / 48\,GB 上仅用 Apple Metal 加速时，实际运行时间约为 8\,min。
...
\subsubsection{设置}
在同样的 75 个可达 koenig 失效用例上，文章评估两个 L3 变体:
\begin{itemize}
...
原始 3.8\% 的可达率较低，但对 307 个未命中用例的检查结果呈现出清晰的规律。payload 使用 BEM~\cite{bemMethodology}: 每个组件声明一个 \texttt{baseClass} 常量（例如 \texttt{const baseClass = 'dashboard'}），并以 \texttt{\{`\$\{baseClass\}\_\_add-button`\}} 这样的模板字面量写出 className。测试引用\emph{渲染后}的类 \texttt{.dashboard\_\_add-button}，这对一个纯字面量的 AST 抽取器是不可见的。
