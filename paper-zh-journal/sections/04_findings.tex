...
\end{tabular}}
\\[2pt]
{\scriptsize $n=75$ 个可达 koenig 失效用例。Ground truth 目标已从候选集中移除；任何被给出的可解析选择器即为一次隐性误修复。78.7\%（59/75）的 Wilson 95\% 置信区间约为 [68.1\%, 86.4\%]。在 \texttt{temperature}=0 的确定性解码下，两提示变体在 75 个用例上逐例标签一致（故聚合计数与逐用例结果均相同，$\Delta=0$）。}
\end{table}
...
\paragraph{解读。}
两项手段均成本低廉（一次正则优先级调整、一段约 10 行的后置过滤）、完全确定性、无需任何模型再训练。强锚点后置过滤器独立触发 2 次且两次均正确，消除了“LLM 选中正确候选但在生成选择器时挂到了不稳定锚点上”这一失败模式。需要明确的是，本文给出的是 v0 与 v1（两手段\emph{叠加}）的端到端对比，尚未拆分“仅 testId 加权检索”与“仅强锚点过滤”的独立贡献；因此“两个手段各自的边际增益”属未分离归因，严格的二因子析因消融（factorial ablation，含 McNemar 配对检验）留作后续工作。在此前提下，5 增 0 减的帕累托改进仍具有较强的可复现性。该结果与 F1 相互呼应：在确定性介入有效之处，提示级介入往往力有未逮。
