...
\item \textbf{第二项发现：两个低开销确定性手段带来零回归的局部改进。}HealReact 的 L1 静态基底在 koenig 上可达 80.6\%（75/93）的真实失效目标；在可达用例上，L3 修复器的 top-1 选择器精确匹配率为 77.3\%（58/75）。进一步地，testId 加权检索与强锚点后置过滤经二因子析因分离后各带来边际改进、合计 +3 个新正确用例且零回归（方向一致，但在 $n=75$ 下 McNemar 未达显著）。
