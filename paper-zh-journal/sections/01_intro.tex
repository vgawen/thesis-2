...
\item \textbf{第三项发现：跨应用泛化受锚点文化差异的制约。}在另一 React+Playwright 代码库（\texttt{payloadcms/payload}，319 个失效配对）上，由于其采用 BEM 风格的模板字面量类名、当前静态抽取器无法对其求值，原始可达率从 koenig 的 80.6\% 跌至 3.8\%。本文将其诊断为一处明确且可修复的盲区，而非对方法本身的否证。
\end{itemize}
