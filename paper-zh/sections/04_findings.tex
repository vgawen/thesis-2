...
\paragraph{设置。}
对 75 个\emph{可达}的 koenig 失效用例（即在失效 commit 处 LocatorSheet 中存在 ground truth 元素的用例），我们以\emph{相同}的提示与检索重跑 L3 修复器，但把 ground truth 元素\emph{强制移除}于 LocatorSheet 与候选集之外。模型在字面意义上无法选中正确答案。正确响应是放弃（\texttt{CANDIDATE\_IDX: -1}）；任何给出的可解析选择器都是一次会沉默地把测试改写到错误元素上的隐性误修复。

我们比较两个提示变体:
\begin{enumerate}
\item \textsc{Vanilla}: 与 L3 baseline 相同的系统提示。
...
\textsc{Vanilla = Abstain} 这一等同性是更具诊断性的发现: 一个 7B 代码模型在此任务上不会因软提示守门规则而改变其可观察行为。任何稳健的放弃机制必须位于模型\emph{之外}: 确定性的检索分数阈值、锚点类别成员规则，最终是一个可以证伪通过补丁的行为 oracle。这从经验上动机化了把 HealReact 的 L4 层（行为重放验证）作为下一层来评估，而非倚赖提示级别的安全网。

\subsection{F2: L3 baseline 与两个廉价确定性杠杆}
\label{sec:f2}

\paragraph{设置。}
在同样的 75 个可达 koenig 失效用例上，我们评估两个 L3 变体:
\begin{itemize}
\item \textsc{v0（baseline）}: 候选按与断裂选择器的 token 重叠检索；系统提示要求模型偏好 testId。
...
\paragraph{解读。}
两个杠杆都廉价（一次正则优先级调整、一段约 10 行的后置过滤）、确定性、不需要任何模型再训练。强锚点过滤器单独触发两次且两次都正确；它消除了"LLM 选对了候选但写选择器时挂到了不稳定锚点上"这种失败模式。帕累托改进（5 增 0 减）是审稿人最快接受的那类证据。该发现与 F1 呼应: \emph{确定性}介入能推动指针的地方，\emph{提示级}介入推不动。

\subsection{F3: 跨应用泛化与 BEM 盲区}
\label{sec:f3}

\paragraph{设置。}
任何单一应用试点都可能被审稿人合理质疑学到了应用特定的约定。我们 sparse-checkout \texttt{payloadcms/payload}~\cite{payloadCMS} 的 HEAD（ReproBreak 中第二大的 Playwright cohort，319 个失效配对），在 \texttt{packages/ui/src} 上运行未修改的 L1 抽取器（产出 457 条记录）。对每个失效配对，我们用 \texttt{new\_locator} 字符串去解析这个 sheet（HEAD 可达率作为代理；逐 commit 配对不在本文范围）。
...
\paragraph{解读。}
这不是对 HealReact L1 设计的否证；它识别出一处范围明确的盲区，其修复是一个确定性的 AST pass: 一个 \texttt{baseClass} 解析器，回溯到常量声明并展开模板字面量（外加一个 CSS Modules~\cite{cssModules} 等价的展开器）。基于对 307 个未命中用例的非系统采样，我们\emph{假设}修复后原始可达率会显著抬升 —— 但这是假设而非测量，我们只在 roadmap（\S\ref{sec:roadmap}，R2）中报告它。3.8\% 是当前抽取器下诚实的原始测量。该发现的贡献是这条可证伪的跨应用泛化论断本身: 锚点文化的多样性（\texttt{data-testid} vs 自定义 \texttt{data-*} vs BEM 模板字面量）是定位器修复工具泛化能力的关卡，任何在单一应用上 benchmark 的工具都很可能高估其泛化性。
